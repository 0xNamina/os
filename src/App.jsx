...
const getGenericABI = () => {
  ...
  const executeMint = async (walletInfo, provider) => {
    ...
    const mintValue = ethers.parseEther(price);
    ...
    // Try detected mint function first
    try {
      if (mintFunc.hasQuantity) {
      ...
      const fallbackMethods = [
        { name: 'mint', hasQty: false },
        { name: 'publicMint', hasQty: false },
        { name: 'mint', hasQty: true },
        { name: 'publicMint', hasQty: true },
        { name: 'claim', hasQty: false },
      ];
      
      let success = false;
      for (const method of fallbackMethods) {
        try {
          addLog(`Trying ${method.name}(${method.hasQty ? config.mintQuantity : ''})...`, 'info');
          
          if (method.hasQty) {
            tx = await contract[method.name](config.mintQuantity, txOptions);
          } else {
            tx = await contract[method.name](txOptions);
          }
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-slate-800/50 backdrop-blur rounded-2xl p-6 shadow-xl border border-slate-700">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">
                    RPC URL {collectionInfo && <span className="text-purple-400">({collectionInfo.chainInfo.name})</span>}
                  </label>
                  <input
                    type="text"
                    value={config.rpcUrl}
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    Get free RPC: Alchemy, Infura, QuickNode, Ankr
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between pt-2 border-t border-slate-700">
                    ...
                    <span className="text-green-400 font-bold">
                      {(parseFloat(collectionInfo.price) * config.mintQuantity + parseFloat(wallets[0]?.gasEstimate || '0.003')).toFixed(6)} {collectionInfo.chainInfo.symbol}
                    </span>
                  </div>
